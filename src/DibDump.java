 ...
 * Rev: 12/02/21  Added a warning message if the bmpFileHeader_biSize is not the expected value of 40 which which means the file has structures we cannot interpret and so we skip over them
 *                and then set bmpFileHeader_biSize to 40 so the output image has the correct header content. 
 *
 * Classes in the file:
 *  RgbQuad
 *  DibDump
...
public class DibDump {
   ...
   public static void main(String[] args) {
      ...
      if (args.length > 0)
         ...
         inFileName = "../photos/IMG_2253.bmp";
      ...
      */
         ...
         if (bmpInfoHeader_biSize != 40) {
            System.out.printf("\nWARNING! bmpInfoHeader_biSize size is not 40 as expected! Continuing to process anyway...\n\n");

            for (int ii = 0; ii < bmpInfoHeader_biSize - 40; ++ii) //We need to skip over the next bmpInfoHeader_biSize - 40 bytes since we have no idea what they are
            {
               in .readByte();
            }

            bmpInfoHeader_biSize = 40; // Must fix the size to thew correct value for our header
         }
      ...
      int xNumerator = 0; // pel * c
      int yNumerator = 0; // pel * r

      int denom = 0; // sum of pels (represent masses)

      for (int r = 0; r < bmpInfoHeader_biHeight; r++) // write over the rows (in the usual inverted format)
      {
         for (int c = 0; c < bmpInfoHeader_biWidth; c++) // and the columns
         {
            pel = imageArray[r][c];
             ...
             * ANIRUDH:
             */
            pel = dibdumper.colorToGrayscale(pel); // Convert to grayscale
            byte leastSigByte = (byte) (pel & 0xFF); // Get least significant byte
//            System.out.println(pel + " leastSigByte: " + leastSigByte);
            pel = leastSigByte;
            pel = ~pel; // Do one's complement
            ...
            pel = dibdumper.colorToGrayscale(pel); // convert back to grayscale

            imageArray[r][c] = pel;
         }
      } // for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)

      int xCenter = xNumerator/denom;
      int yCenter = yNumerator/denom;

      System.out.println("xCenter: " +xCenter + " ycenter:" + yCenter);

      int cropx = 90;
      int cropy = 90;
      int[][] croppedImage = new int[cropx][cropy];

      for (int r = yCenter - cropy/2; r < yCenter + cropy/2; r++)
      {
         for (int c = xCenter - cropx/2; c < xCenter + cropx/2; c++)
         {
            System.out.println("r: " + r + " c: " + c);
            if (r >= 0 && c >= 0 && r < imageArray.length && c < imageArray[0].length)
               croppedImage[r][c] = imageArray[r][c];
            else
               throw new RuntimeException("R AND C ARE WRONG. R: " + r + " C: " + c);
         }
      }
